...
export function formatTotalCost(): string {
  ...
  const arisCreditsDisplay = formatArisCredits(getArisCredits())

  return chalk.dim(
    `Total cost:            ${costDisplay}\n` +
      `Aris credits:          ${arisCreditsDisplay}\n` +
...
// Conversion factor from USD to Aris credits. 1 USD spent = 1000 credits.
// This keeps free-tier users (1000 credits) usable for ~$1 of API spend.
const ARIS_CREDITS_PER_USD = 1000
...
export function addToTotalSessionCost(
  ...
  // Deduct the spend from the user's Aris credits balance. Rounded up so
  // tiny calls still cost at least 1 credit; clamped to 0 in spendArisCredits.
  const creditsToSpend = Math.ceil(cost * ARIS_CREDITS_PER_USD)
  if (creditsToSpend > 0) {
    spendArisCredits(creditsToSpend)
  }
